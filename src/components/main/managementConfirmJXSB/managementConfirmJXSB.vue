  ...
  <div>
    ...
    <el-tabs v-model="activeTab" @tab-click="handleClick" style="margin: 20px;">
      <el-tab-pane label="全部" name="pane-0">
        <ManagementTable :tableData="allTableData.data" :state="true" :loading-v="loading"/>
        <!--    start-分页器-->
      ...
      <el-tab-pane label="待确认" name="pane-1">
        <ManagementTable :tableData="waitTableData.data" :loading-v="loading"/>
        <!--    start-分页器-->
      ...
      <el-tab-pane label="已确认" name="pane-2">
        <ManagementTable :tableData="doneTableData.data" :state="true" :loading-v="loading"/>
        <!--    start-分页器-->
...
<script>
  ...
  export default {
    name: "managementConfirmJXSB",
    components: {Pager, ManagementTable},
    data() {
      return {
        loading: false,
    ...
    methods: {
      ...
      async allTableData_init(pageSize = 20) { //初始化全部数据列表
        await this.$axios.post('search/management_aggregation/list',
          {
      ...
      async doneTableData_init(pageSize = 20) { //初始化评估完成列表数据
        ...
        await this.$axios.post('search/management_aggregation/list',
          ...
          this.doneTableData.data = response.data.data.records.slice(0, this.pageSize);
