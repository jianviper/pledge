<template>
  <div>
    <searchAndTable :listUrl="lurl" :roleType="'HGF'" @returnRowData="acceptRowData" :key="timer"/>
    <!--    添加资方数据窗口-->
    <el-dialog title="添加回购方" :visible.sync="dialogVisible" width="500px" @close="close_dialog()" :close-on-click-modal='false'>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        addflag: true,
        lurl: 'baseInfo/agency/buyBackList',
        timer: '',
        ...
        rules: {
          ...
          industry: {required: true, message: '请选择支持产业', trigger: 'change'},
        }
      }
    },
    methods: {
      ...
      add_submit() {
        ...
        this.$refs['agencyFormData'].validate((valid) => {
          if (valid) {
            this.$axios.post('baseInfo/agency/saveBuyBack', this.agencyFormData).then((response) => {
